...
function setup() {
  ...
  if (itemSh.getLastRow() === 0) {
    itemSh.appendRow(['id','name','category','price','slogan','description','img','tag']);
...
function doPost(e) {
  ...
  try {
    ...
    if (action === 'addItem') {
      ...
      sh.appendRow([item.id, item.name, item.category, item.price, item.slogan || '', item.description || '', item.img || '', item.tag || '']);
    ...
    if (action === 'updateItem') {
      updateRowWhere(ss.getSheetByName(ITEM_SHEET), 'id', data.id, data.patch);
      return jsonOut({ ok: true });
    }
